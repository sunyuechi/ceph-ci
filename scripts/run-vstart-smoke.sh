#!/usr/bin/env bash
#
# L3 (experimental): bring up a single-node cluster from the build-check build tree, do
# basic rados/rbd/cephfs IO, tear it down. Runs in the openRuyi build container via
# bwc's `custom` step. Failures here are a known-gap signal, not a build-check regression.
#
...
# Reads the build-check CI's checkout (run-build-check.sh builds into ${WORKDIR}/build-check/ceph).
CEPH_SRC="${WORKDIR}/build-check/ceph"
ENGINE="${CONTAINER_ENGINE:-podman}"

[ -d "${CEPH_SRC}/build" ] || { echo "ERROR: no build tree at ${CEPH_SRC}/build; run the build-check build first" >&2; exit 1; }
